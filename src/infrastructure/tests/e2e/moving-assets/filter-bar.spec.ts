...
test("filter bar is visible", async ({ page }) => {
  await expect(page.getByText("3 filters are currently applied.")).toBeVisible();
});

test("open the filters drawer from the filter bar", async ({ page }) => {
  await page.locator('button:has-text("Filters")').first().click();
  await expect(page.getByRole("heading", { name: "Moving Assets Filters", exact: true })).toBeVisible();
});
