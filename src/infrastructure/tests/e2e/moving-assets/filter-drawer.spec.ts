...
test("close the filters drawer", async ({ page }) => {
  ...
  await expect(page.getByText("Filters3", { exact: true })).not.toBeVisible();
});
...
test("default filters are selected", async ({ page }) => {
  ...
  await expect(page.getByRole("option", { name: "Moving", exact: true }).getByRole("checkbox")).toBeDisabled();
...
test("apply and check filters", async ({ page }) => {
  // Moving motion status should be disabled
  const selector = page.getByPlaceholder(filterConfig.motionStatus.placeholder);
  await selector.click();
  await expect(page.getByRole("option", { name: "Moving", exact: true }).getByRole("checkbox")).toBeDisabled();

  await expect(page.getByText("Moving Assets Filters")).toBeVisible();
