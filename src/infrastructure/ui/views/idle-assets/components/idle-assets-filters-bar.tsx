...
export function IdleAssetsFiltersBar() {
  const { asset, location, sensor, operational } = useIdleAssetFilters(true);
  const recents = useIdleAssetRecentFilters();
  const { setFilter, setRecentFilter, setPinnedFilter, isFilterPinned } = useIdleAssetActions(true);
  const count = useIdleAssetFiltersCounts(true);
  const { filtersPinnedCount, activeFiltersCount } = useIdleAssetActions(true);
