...
import { useIdleAssetFiltersCounts } from "@/store/idle-asset.store";
...
export function IdleAssetsFiltersPanel({ view }: { view: View }) {
  const baseCounts = useIdleAssetFiltersCounts();
  const viewCounts = useIdleAssetFiltersCounts(view === "idle-assets");

  const counts = view === "idle-assets" ? viewCounts : baseCounts;

  return (
    <>
      <Feature flag="idle-assets" option="showTextSearch">
