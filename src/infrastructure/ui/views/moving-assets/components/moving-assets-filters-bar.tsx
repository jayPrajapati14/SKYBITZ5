...
import { getLastReportedLabel } from "@/domain/utils/datetime";
...
export function MovingAssetsFiltersBar() {
  const { asset, location, sensor, operational } = useMovingAssetFilters(true);
  const recents = useMovingAssetRecentFilters();
  const { setFilter, setRecentFilter, setPinnedFilter, isFilterPinned } = useMovingAssetActions(true);
  const count = useMovingAssetFiltersCounts(true);
  const { filtersPinnedCount, activeFiltersCount } = useMovingAssetActions(true);
  ...
  return (
    <>
      <Feature flag="moving-assets" option="showTextSearch">
