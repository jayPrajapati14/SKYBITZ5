...
import { useMovingAssetFiltersCounts } from "@/store/moving-asset.store";
...
export function MovingAssetsFiltersPanel({ view }: { view: View }) {
  const baseCounts = useMovingAssetFiltersCounts();
  const viewCounts = useMovingAssetFiltersCounts(view === "moving-assets");

  const counts = view === "moving-assets" ? viewCounts : baseCounts;
