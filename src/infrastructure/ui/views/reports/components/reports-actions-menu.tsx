...
export function ReportsActionsMenu({ view, totalResults = 0 }: ReportsActionsMenuProps) {
  ...
  const { landmark, asset, location, display } = useYardCheckFilters(true);
  ...
  const onConfirmDownloadReport = async () => {
    ...
    if (view === "yard-check") {
      ...
      const assets = await getYardCheckAssetsForExport(options);
  ...
  return (
    <>
      ...
      <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleClose}>
        {view !== "reports" ? (
          <MenuItem onClick={totalResults > 0 ? handleDownloadCSV : undefined} disabled={totalResults === 0}>
            <ListItemIcon>
              <GridOn />
            </ListItemIcon>
