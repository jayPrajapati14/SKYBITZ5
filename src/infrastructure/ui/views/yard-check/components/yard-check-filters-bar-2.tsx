...
export function YardCheckFiltersBar() {
  const { asset, location, sensor, operational } = useYardCheckFilters(true);
  ...
  const { filtersPinnedCount, activeFiltersCount } = useYardCheckActions(true);
  ...
  return (
    <>
      ...
      {/* {isFilterPinned("sensor", "volumetricStatuses") ? (
        <Filters.Dropdown>
          <Filters.Chip count={sensorVolumetricStatusCount} label="Volumetric Cargo Status" />
          <Filters.Popover>
            <SensorFilterVolumetricCargoStatus
              volumetricStatus={sensor.volumetricStatuses ?? []}
              onChange={(volumetricStatuses) => setFilter("sensor", "volumetricStatuses", volumetricStatuses)}
              isPinned={isFilterPinned("sensor", "volumetricStatuses")}
              onPinChange={(status) => setPinnedFilter("sensor", "volumetricStatuses", status)}
            />
          </Filters.Popover>
        </Filters.Dropdown>
      ) : null} */}
